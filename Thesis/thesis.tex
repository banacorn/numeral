...
% \subsubsection{Zeroless representation and bijective numeration}
...
\begin{center}
    ...
    \end{tabular}
\end{center}
% To model this, we introduce another generalization on the number of digits that
% would be denoted as $ \#digit $. Digits now range from \textit{offset} to
% \textit{offset + \#digit - 1}
% Formerly a system would have exactly \textit{base} number of digits
% that ranges from \textit{offset} to \textit{offset + base - 1}, but if we allow
% a system to have more digits,

% Now that there are more than one way of representing a number

% Such representations are known as redundant numeral systems.

% We will explain why it makes perfect sense to include these systems into our
% representation.

% In a numeral system where $ base \textgreater 1 $, operations such as addition
% would take $ O(log{}n)$ for some number $ n $. But in


% A number $ n $ of some numeral system with a $ base >= 2 $ would have length

% A numeral system is said to be \textit{redundant} when there is more than one way
% to represent a number. We could make a numeral system redundant by adding more
% digits. Take binary numbers for instance, if we allow $ 2 $ to be one of the digits,
% then the decimal number $ 5 $ can now be represented as $ 101 $ or $ 21 $.
%
% But what is redundancy good for? It turns out that some data structures can actually
% be modeled with redundant numeral systems. 2-3 tree is one such example.

%  It turns out that, some data structures, such as
% the 2-3 tree, can be modeled with a numeral system.

 % there are two ways to represent the number $ 5 $

% The decimal numeral system is one such instance, we can
% find another notation of the same number just by adding leading zeros.
%
% $$
%     2016, 02016, 002016, ...
% $$

% There's another way of making a numeral system redundant. If we take the binary
% numeral system and tweak it a bit, adding another digit {2}.


% If we extend the set of digits of binary numeral system

% We can make the binary numeral system redundant by adding one more digit.
% Making D = {0, 1, 2}. Then the number (5)10 can be represented as (101)2 or (21)2 .




\subsubsection{From numeral systems to numbers}
% To make things clear, now we distinct \textit{notations} from \textit{numbers}.
% The former is syntactic, which refers to a series of digits, they bear no meanings.


% and we the function that converts notations to numbers an \textit{evaluation}.
