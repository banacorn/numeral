...
It is difficult to see what is going on in between these clauses, let alone
proofs that involves dozens of {\lstinline|trans|}.
Things only get worse as propositions get more complicated.

Fortunately,
% But it is not an easy task either to read or to write, for example, the line we
% have just proven.

% But it is not an easy task either to read or to write the proof


% We see that proofs are composable and can be written in a modular fashion, just
% like programs. But it is not an easy task either to read or to write the proof
