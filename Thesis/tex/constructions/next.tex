...
Given a numeral {\lstinline|_ ∷ xs|} and suppose that the next numeral of
{\lstinline|xs|} is {\lstinline|xs'|}

\begin{center}
    \begin{adjustbox}{max width=\textwidth}
        \begin{tikzpicture}
            % the frame
            \path[clip] (5.5, -1.5) rectangle (17.5, 2.5);

            % the body
            \foreach \i in {0,...,2} {
                \foreach \j in {0,...,5} {
                    \draw[ultra thick, draw=gray] ({\i*8+\j+0.05}, {\i*0.6-0.8}) rectangle ({\i*8+\j+0.95}, {\i*0.6-0.4});
                };
            };

            \draw[ultra thick, loosely dotted] (7.5,-1) -- (7.5,1)
                node[above] {{\lstinline|⟦ xs ⟧ × b|}};

            \draw[ultra thick, loosely dotted] (15.5,-1) -- (15.5,1)
                node[above] {{\lstinline|⟦ xs' ⟧ × b|}};

            \draw[ultra thick, decoration={brace,mirror,raise=10},decorate]
                (8,-0.2) -- node[below=0.5] {{\lstinline|d|}} (14,-0.2);
        \end{tikzpicture}
    \end{adjustbox}
\end{center}

If we look closely, we can see that the gap comes from the difference between
the number of digits {\lstinline|d|} and {\lstinline|⟦ xs' ⟧ × b - ⟦ xs ⟧ × b|}.

\begin{center}
    \begin{adjustbox}{max width=\textwidth}
        \begin{tikzpicture}
            % the frame
            \path[clip] (25, -2.5) rectangle (35, 3);

            % the body
            \foreach \i in {0,...,2} {
                \foreach \j in {0,...,5} {
                    \draw[ultra thick, draw=gray] ({\i*16+\j*2+0.1}, {\i*1.2-1.6}) rectangle ({\i*16+\j*2+1.9}, {\i*1.2-0.8});
                };
            };

            \draw[ultra thick, loosely dotted] (31,-1) -- (31,2)
                node[above] {{\lstinline|⟦ xs' ⟧ × b|}};

            \draw[ultra thick, loosely dotted] (27,0.4) -- (27,2)
                node[above] {{\lstinline|⟦ xs ⟧ × b + d|}};

            \draw[ultra thick, loosely dotted] (27,-0.4) -- (27,-1);

            \draw[ultra thick, decoration={brace,mirror,raise=10},decorate]
                (27,-1) -- node[below=0.5] {{\lstinline|the gap|}} (31,-1);
            % \draw[ultra thick, decoration={brace,mirror,raise=10},decorate]
            %     (8,-0.2) -- node[below=0.5] {{\lstinline|Digit-toℕ x o|}} (15,-0.2);
        \end{tikzpicture}
    \end{adjustbox}
\end{center}
