...
Some of the more advenced \scm{advanced} constructions (such as views and universes) used in
...
And it is often difficult, if not impossible, to develop and prove a theorom \scm{theorem} at one stroke.
Just like programming, the process is incremental.
So Agda allows us to leave some "holes" \scm{``holes''} in a program, refine them gradually, and
...
Then we may ask Agda to pattern match on {\lstinline|x|} and rewrite the program for us: \scm{Is \lstinline|Int| constructed by \lstinline|zero|
and \lstinline|suc|?}
...
the problem for us, if it is not too difficult. \scm{This sentence implies that
Agda is able to fill in \lstinline|true| and \lstinline|false| for
\lstinline|is-zero|. Is this possible?}
...
This is basically what pragramming \scm{programming} and proving things looks like in Agda.

\scm{In formal papers, try avoid using abbreviation such as
``there's'', ``it's'', etc. Say ``there is'' and ``it is.''}
...
\end{enumerate}
\scm{I would just say ``{\lstinline|true|} has type {\lstinline|Bool|}}'',
etc.}

\paragraph{Pattern matching}
% [draft]
Similar to Haskell, datatypes are eliminated with \scm{by?} pattern matching.

Here's \scm{Here is} a function that pattern matches on {\lstinline|Bool|}.
...
Agda is a \textit{total} language, that means \scm{which means that} partial functions are not valid constructions.
Functions are guarantee \scm{guaranteed} to terminate and will not crash on all possible inputs.
The following example won't be accecpted \scm{accepted} by the type checker \scm{termination checker?}, because the case
{\lstinline|false|} is missing.
\scm{I think the right tune is ``programmers are obliged to convince Agda that
...
\scm{Not sure why this sentence is here...}

\paragraph{Inductive datatype} Let's move on to a more interesting datatype with
\scm{an} inductive definition. Here's the type of natural numbers.
% [draft]
\begin{lstlisting}
data ℕ : Set where
    zero : ℕ
...
Agda also accepts arabic \scm{decimal?} literals if the datatype {\lstinline|ℕ|} complies with
...
function is a function whose result type \scm{remove ``result''?} may depend on the value of an argument \scm{its argument(s)?}.
...
For example, if there's term whose type is \scm{given a term whose type is...}  {\lstinline|Vec Bool zero|}, then
...
\scm{It is better to change the tune a bit and explain that what
actually happens is that, when {\lstinline|head|} is applied to
some list {\lstinline|xs|}, the programmer is obliged to convince
Agda that {\lstinline|non-empty xs|} reduces to {\lstinline|⊤|}.}
...
when {\lstinline|non-empty xs|} evaluates to {\lstinline|⊥|} \scm{you mean {\lstinline|⊤|}?}, {\lstinline|tt|}
...
Contrary to classical logic, where propositions evaluates to truth values. \scm{no verb in this sentence}
...
Suppose we want to know if a natural \scm{number} is even or odd. We know that {\lstinline|zero|}
...
is \textit{extensional}. \scm{What does this mean?} However, it could be expressed as a \textit{proposition}
...
\scm{It sounds like computational equality and propositional
...
\scm{should the following {\lstinline|eq|} be refined to {\lstinline|refl|}?}
\begin{lstlisting}
sym' : (a .a : ℕ) → a ≡ a → a ≡ a
sym' a .a eq = ?
\end{lstlisting}
...
\footnote{Some primitives that require special treatments, such as IO, are take cared with \scm{taken care of by}
language pragmas exposed \scm{provided} by Agda.}
