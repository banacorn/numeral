...
materials cited above, covering the \scmsubst{part}{parts} (and only the \scmsubst{part}{parts}) we need in this thesis.
...
We assume that readers have some basic understanding of Haskell\scmsubst{, and those}{. Readers}
...
which states that: \scm{``propositions are types''} and \scm{``proofs are programs.''}
...
complete the proofs \scm{``hole by hole''}.
...
\scm{``what's the type of the goal?''}, \scm{``what's the context of this case?''}, etc.
...
The decimal number \scm{``4''} is represented as {\lstinline|suc (suc (suc (suc zero)))|}.
...
value has to \textit{named}\scmsubst{, in}{. In} Agda we write:
...
means \scm{``I don't bother naming it''}.
...
{\lstinline|id Bool true|} evaluates to {\lstinline|true|}\scm{.}
...
Parameters are \textit{parametric}, in the sense that, they have no effect on the \scm{``shape''} of a datatype.
...
Saying that two things are \scm{``equal''} is a notoriously intricate topic in type theory.
...
The standard library is not \scm{``chartered''} by the compiler or the type checker,
