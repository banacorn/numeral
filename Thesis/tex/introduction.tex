...
Take the notation \scm{``11''} for example; it could have several meanings.
...
\scm{1. I'd start the paragraph label with captial letter (Problem).
But you can have your own style as long as it is consistent in this thesis.
\\
2. I still feel that ``lead to inconsistency'' is a bit vague and
imprecise. Is it right to say something like ``we have to generalize our representation to
allow this'' (and mention in which section you are going to do so)?
}
...
\scm{Why not? A brief explanation?}
...
addition on these digits \scmsubst{must be}{can be implemented in}
\textbf{constant time}.
...
\scm{It might be better if we swap the order of the first two sentences.
(Since a system... However, the greater a number is...)}
