...
Take the number "2016" \scm{use ``2016'' for quotes} for example:
...
In this case, $ 6 $ is called \scm{the} \textit{least significant digit},
...
its semantics can only be carried out by \textit{converting} to some other syntax. \scm{not sure what ``carried out'' means...}
...
\scm{A numeral system of base $n$ has exactly $n$ digits, which are assigned
...
\scm{You left a dangling question here: will this inconsistency be resolved?}
...
\scm{Immediately I was thinking: because the ALU is not a complete
implementation of arithmetics!}
...
\scm{Hmm... a bit awkward here. I'd say that the circuits fail to capture
real arithmetics, rather than the other way round.}
