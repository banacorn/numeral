...
\footnote{In fact, all of these proofs (hence programs) can be generalized \scmsubst{with}{to}
...
The proof has to be \scmsubst{break}{``broken'', or ``split''} into two steps:
...
With equational reasoning, we can see how an expression equates \scmsubst{with}{to} another,
step by step, justified \scmsubst{with}{by} theorems. The first and the last \scmsubst{step corresponds}{steps correspond} to
...
part \ref{eq-part3}, while remaining equal \scm{this sentence has no verb}.
...
We could ask {\lstinline|cong|} to \scm{``target''} the part to substitute by supplying a
function like this:
...
\scm{ideally this line should start with ``which'', with no
identation.}
...
defined as:
\scm{Awkward: {\lstinline|_≤_|} (less-than-or-equal-to) is an
instance of a preorder, but here it sounds like it represents all
preorders.}
\begin{lstlisting}
...
Here are some other binary relations \scmsubst{than}{that} can be defined with {\lstinline|_≤_|}.
...
We can only substitute part of an expression when the function is \textit{monotonic}. \scm{what about ``Given \lstinline|x ≤ y|, we can substitute
\lstinline|f x| by \lstinline|f y| using a proof that \lstinline|f| is monotonic.''}
...
\subsection{Relational Propositions [work in process]}
