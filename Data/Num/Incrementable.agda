...
-- Incrementable? {b} {d} {o} xs xs! with boundedView b d o
-- Incrementable? xs xs! | IsBounded (Base≡0 d o) = {!   !}
-- Incrementable? xs xs! | IsBounded (HasOnly0 b) = {!   !}
-- Incrementable? xs xs! | IsntBounded (Digit+Offset≥2 b d o d+o≥2) = {!   !}
-- Incrementable? xs xs! | IsntBounded (HasNoDigit b o) = {!   !}
...
--     → suc (Fin.toℕ x) ≡ suc d
--     → ¬ (Incrementable (x ∷ xs))
-- Incrementable-lemma-4 x xs max (∙ , ())
-- Incrementable-lemma-4 {d} {o} x xs max (x' ∷ xs' , q) =
...
-- -- Incrementable-lemma-5 {b} {d} {o} x xs xs' max redundant greatest = {!   !}
--     -- begin
...
--     -- -- max : toℕ xs' ≡ suc (toℕ xs)
--     -- ≡⟨ cong (λ w → (Fin.toℕ x ∸ b) + o + w * suc b) max ⟩
...
-- --     → Incrementable (x ∷ xs)
-- --     → ⊥
-- -- Incrementable-lemma-7 x xs greatest ¬max (∙ , ())
-- -- Incrementable-lemma-7 x xs greatest ¬max (y ∷ ys , claim) = {! ⟦xs⟧<⟦ys⟧  !}
--     -- ¬max : Σ[ xs' ∈ _ ] toℕ xs' ≡ suc (toℕ xs) → ⊥
...
-- --     {!   !}
-- -- □
-- -- Incrementable-lemma-11 x xs greatest ¬max ¬redundant (y ∷ ys , claim) | yes max = {!   !}
-- -- Incrementable-lemma-11 x xs greatest ¬max ¬redundant (y ∷ ys , claim) | no ¬max = {!   !}
...
-- -- Incrementable? {suc b} {suc d} (x ∷ xs) | yes greatest | no ¬incr | yes max with Maximum? xs
-- -- Incrementable? {suc b} {suc d} (x ∷ xs) | yes greatest | no ¬incr | yes max | yes max
-- --     = no {!   !}
-- -- Incrementable? {suc b} {suc d} (x ∷ xs) | yes greatest | no ¬incr | yes max | no ¬max
-- --     = yes {!   !}
-- --     -- = yes ({!   !} ∷ next xs {!   !} , {!   !})
