...
mutual
    Gapped#0 : ∀ b d o
        → (proper : 2 ≤ suc (d + o))
        → Set
    Gapped#0 b d o proper = suc d < Digit-toℕ (LCD d o proper) o * suc b
    ...
    Gapped#0? :  ∀ b d o
        → (proper : 2 ≤ suc (d + o))
        → Dec (Gapped#0 b d o proper)
    Gapped#0? b d o proper = suc (suc d) ≤? Digit-toℕ (LCD d o proper) o * suc b
    ...
    Gapped : ∀ {b d o}
        ...
        → (proper : 2 ≤ suc (d + o))
        → Set
    Gapped {b} {d} {o} (x ∙)    proper = Gapped#0 b d o    proper
    ...
    next-number-Proper-NotGapped {b} {d} {o} (x ∙) greatest proper gapped
        = digit+1-n x greatest (Digit-toℕ carry o * suc b) lower-bound ∷ carry ∙
        where
            carry : Digit (suc d)
            carry = LCD d o proper

            lower-bound : Digit-toℕ carry o * suc b > 0
            lower-bound =
                start
                    1
                ≤⟨ s≤s z≤n ⟩
                ...
                ≤⟨ *n-mono (suc b) (m≤m⊔n 1 o) ⟩
                    (1 ⊔ o) * suc b
                ≈⟨ cong (λ w → w * suc b) (sym (LCD-toℕ d o proper)) ⟩
                    Digit-toℕ carry o * suc b
                □
    ...
    next-number-is-greater-Proper (x ∙) proper | NotGapped b d o greatest ¬gapped =
        ...
        ≈⟨ {!   !} ⟩
            {!   !}
        ≈⟨ {!   !} ⟩
            Digit-toℕ (digit+1-n x greatest (Digit-toℕ (LCD d o proper) o * suc b) {!   !}) o + (Digit-toℕ (LCD d o proper) o) * suc b
        □
        -- start
        --     suc (Fin.toℕ x + o)
        -- ≈⟨ sym (m∸n+n≡m {_} {(1 ⊔ o) * suc b} upper-bound') ⟩
        --     suc (Fin.toℕ x + o) ∸ (1 ⊔ o) * suc b + (1 ⊔ o) * suc b
        -- ≈⟨ cong (λ w → suc (Fin.toℕ x + o) ∸ (1 ⊔ o) * suc b + w * suc b) (sym (LCD-toℕ d o proper)) ⟩
        --     suc (Fin.toℕ x + o) ∸ (1 ⊔ o) * suc b + (Digit-toℕ (LCD d o proper) o) * suc b
        -- ≈⟨ cong (λ w → w + (Digit-toℕ (LCD d o proper) o) * suc b) (sym (digit+1-n-toℕ x greatest ((1 ⊔ o) * suc b) lower-bound upper-bound)) ⟩
        --     Digit-toℕ (digit+1-n x greatest ((1 ⊔ o) * suc b) lower-bound) o + (Digit-toℕ (LCD d o proper) o) * suc b
        -- □
        where
            upper-bound : Digit-toℕ (LCD d o proper) o * suc b ≤ suc d
            upper-bound = ≤-pred (≰⇒> ¬gapped)
            --
            -- lower-bound : 1 ≤ (1 ⊔ o) * suc b
            -- lower-bound = {!   !}
            -- -- lower-bound = start
            -- --         1
            -- --     ≈⟨ refl ⟩
            ...
            -- --     □
            --
            -- upper-bound' : (1 ⊔ o) * suc b ≤ suc (Digit-toℕ x o)
            -- upper-bound' = {!   !}
                -- start
                --     (1 ⊔ o) * suc b
                -- ≤⟨ upper-bound ⟩
...
--             ≡⟨ cong (λ w → w + (1 ⊔ o) * suc b) (digit+1-n-toℕ x greatest ((1 ⊔ o) * suc b) lower-bound upper-bound) ⟩
--                 suc (Fin.toℕ x + o) ∸ (1 ⊔ o) * suc b + (1 ⊔ o) * suc b
--             ≡⟨ m∸n+n≡m upper-bound' ⟩
--                 suc (Digit-toℕ x o)
--             ∎
